...
class Parse5RichTextParser {
    resolveRichTextElement(contentItemCodename, html, elementName, replacement, config, linkedItemIndex = new parse_models_1.RichTextItemIndexReferenceWrapper(0)) {
        // create document
        const documentFragment = parse5_1.parseFragment(html);
        ...
        const resolvedHtml = parse5_1.serialize(documentFragment);
        return {
            links: result.links,
    ...
    processImage(contentItemCodename, elementName, element, replacement, config, result) {
        ...
        const dataImageIdAttribute = attributes.find((m) => m.name === parser_configuration_1.parserConfiguration.imageElementData.dataImageId);
        ...
        // set url of image
        const srcAttribute = attributes.find((m) => m.name === parser_configuration_1.parserConfiguration.imageElementData.srcAttribute);
        if (srcAttribute) {
    ...
    processLink(element, replacement, config, result) {
        ...
        const dataItemIdAttribute = attributes.find((m) => m.name === parser_configuration_1.parserConfiguration.linkElementData.dataItemId);
        ...
        if (urlSlugResult.html) {
            ...
            const linkHtml = urlSlugResult.html
                ...
                : '';
            if (linkHtml) {
                const linkRootNodes = parse5_1.parseFragment(linkHtml).childNodes;
    ...
    processModularContentItem(elementName, element, replacement, config, result, linkedItemIndex) {
        const attributes = element.attrs;
        const dataTypeAttribute = attributes.find((m) => m.name === parser_configuration_1.parserConfiguration.modularContentElementData.dataType);
        const resolvedDataAttribute = attributes.find((m) => m.name === parser_configuration_1.parserConfiguration.resolvedLinkedItemAttribute);
        ...
        if (dataTypeAttribute && !resolvedDataAttribute) {
            ...
            if (dataTypeAttribute.value === 'item') {
                ...
                const dataCodenameAttribute = attributes.find((m) => m.name === parser_configuration_1.parserConfiguration.modularContentElementData.dataCodename);
                ...
                const relAttribute = attributes.find((m) => m.name === parser_configuration_1.parserConfiguration.modularContentElementData.relAttribute);
                ...
                const serializedChildNodes = parse5_1.parseFragment(resultHtml);
                // add child nodes
