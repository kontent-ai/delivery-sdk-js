...
class ElementMapper {
    ...
    mapElement(data) {
        const elementType = kontent_core_1.enumHelper.getEnumFromValue(elements_1.ElementType, data.elementWrapper.rawElement.type);
        if (elementType) {
    ...
    mapRichTextElement(item, elementWrapper, queryConfig, processedItems, processingStartedForCodenames, preparedItems) {
        ...
        return new elements_1.Elements.RichTextElement(elementWrapper, rawElement.modular_content, {
            links: links,
            resolveRichTextFunc: () => resolvers_1.richTextResolver.resolveData(item.system.codename, rawElement.value, elementWrapper.propertyName, {
    ...
    mapUrlSlugElement(elementWrapper, item, queryConfig) {
        ...
        return new elements_1.Elements.UrlSlugElement(elementWrapper, {
            resolveLinkFunc: () => resolvers_1.urlSlugResolver.resolveUrl({
    ...
    getGlobalUrlSlugResolverForType(type) {
        const item = resolvers_1.stronglyTypedResolver.createEmptyItemInstanceOfType(type, this.config.typeResolvers || []);
