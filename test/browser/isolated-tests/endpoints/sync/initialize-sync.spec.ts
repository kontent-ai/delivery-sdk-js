...
import * as responseJson from './initialize-sync.spec.json';

describe('Initialize sync', () => {
    const context = new Context();
    setup(context);

    let response: Responses.IInitializeSyncResponse;

    beforeAll(async () => {
        response = (await getDeliveryClientWithJson(responseJson).initializeSync().toPromise()).data;
    });
