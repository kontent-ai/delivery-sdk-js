...
export function resolveBrowserRichTextInternal(
    ...
    if (!htmlCollection || htmlCollection.length === 0) {
        ...
        for (let i = 0; i < htmlCollection.length; i++) {
            ...
            if (
                ...
                if (!parentElement) {
                    ...
                    if (dataTypeAttribute.value === 'item') {
                        ...
                        newElem.innerHTML = resolveBrowserRichTextInternal(input, linkedItemHtml ?? '', linkedItemIndex).html;
            ...
            if (element.children && element.children.length > 0) {
                processRichTextElement(input, element.children, result, linkedItemIndex);
            }
        }
    }
